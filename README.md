...
Running scripts in `_0_` and `_1_` will prepare all necessary data for GMVAE and `scTAPE` training. The downloaded data will be split into a train data set and a test data set in separate folders for the GMVAE training. Running the scripts `_2_` will prepare the data to train `scTAPE`. The raw scRNA-seq data in the train data will be saved in a different format as a dense matrix. Several pseudo-bulk RNA-seq data will be created by random sampling from the test data and TPM-normalized.      
